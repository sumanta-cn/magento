...
class Cn_Banner_Adminhtml_BannerController extends Mage_Adminhtml_Controller_action
{
		...
		if ($data = $this->getRequest()->getPost()) {
			...
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
		        ...
		        //this way the name is saved in DB
	  			$data['filename'] = 'banner/'.$newfilename;
			}
    ...
    unlink(Mage::getBaseDir('media') . DS . $data['filename']['value']);
			...
			else
			{
				...
				$data['filename'] = $bannerModel->getFilename();
			}
            ...
            echo "test10<pre>";print_r($data);die;
        }
